...
class Database:
    ...
    async def gamma_mode_status(self, bot_id):
        bot = await self.botcol.find_one({'id': int(bot_id)}, {'FTM_GAMMA_MODE': 1})
        return bot.get('FTM_GAMMA_MODE', FTM_GAMMA_MODE) if bot else FTM_GAMMA_MODE

    async def update_gamma_mode_status(self, bot_id, enable):
        await self.botcol.update_one(
            {'id': int(bot_id)},
            {'$set': {'FTM_GAMMA_MODE': enable}},
            upsert=True
        )
