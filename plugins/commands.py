...
from info import CHANNELS, FSUB_PICS, ADMINS, AUTH_USERS, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, REQST_CHANNEL, GRP_LNK, SUPPORT_CHAT_ID, MAX_B_TN, VERIFY, REACTIONS, HOW_TO_VERIFY, PICS, FTMBOTZX_VERIFIED_LOG, SUBSCRIPTION, FTMBOTZX_MOVIE_UPDATE_CHANNEL_LNK, STREAM_MODE, EMOJI_MODE, OWNER_LNK, OWNER_UPI_ID, QR_CODE
...
@Client.on_message(filters.private & filters.command("ftm_gamma") & filters.user(AUTH_USERS))
async def set_ftm_gamma_mode(client, message):
    bot_id = client.me.id
    try:
        option = message.text.split(" ", 1)[1].strip().lower()
        enable_status = option in ['on', 'true']
        ...
        return
    try:
        await db.update_gamma_mode_status(bot_id, enable_status)
        response_text = (
            "<b>🎯 FTM GAMMA MODE ᴇɴᴀʙʟᴇᴅ ✅</b>\n\n<i>Enhanced search functionality with inline buttons will now be available in groups.</i>" if enable_status 
            else "<b>🎯 FTM GAMMA MODE ᴅɪꜱᴀʙʟᴇᴅ ❌</b>\n\n<i>Standard search functionality restored.</i>"
        )
        await message.reply_text(response_text)
    except Exception as e:
        await log_error(client, f"Error in set_ftm_gamma_mode: {e}")
        await message.reply_text(f"<b>❗ An error occurred: {e}</b>")
